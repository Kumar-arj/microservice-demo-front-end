    ...
    stage('Docker Build') {
        ...
        stage('Build Image') {
          docker.withRegistry('https://registry.hub.docker.com', 'dockerhub') {
            ...
            customImage.push()
            def finalImage = docker.build("kumararj/sock-shop-micro-services-front:latest")
            finalImage.push()
        ...
        stage('Helm Chart') {
          container('build') {
            dir('charts') {
              ...
              sh '/usr/local/bin/helm package micro-services-front'
              sh 'curl -v -u $username:$password --upload-file micro-services-front-1.0.tgz http://nexus.k4m.in/repository/sock-shop-helm-local/'
              }
            }
          }
        }
